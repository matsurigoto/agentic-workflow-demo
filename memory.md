...
10. **due_date stored as String** - typed date column would allow full DB-side date filtering (schema migration needed, needs issue first)
11. **NotificationService connection/stream leaks** - HttpURLConnection and OutputStream never closed in sendSlackNotification()
...
- 2026-08-07 run 8: PR - `perf: fix thread-unsafe singleton and InputStream leak in ConfigManager`

## Backlog Cursor
Next area: due_date stored as String (item 10, needs issue first) or NotificationService leaks (item 11)

## Last Run Tasks
- 2026-08-07 run 8: Task 3 (implement - ConfigManager thread-safety + resource leak), Task 7 (monthly summary)
