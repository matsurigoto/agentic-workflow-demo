...
- ConfigManager: singleton with race condition; InputStream leak now fixed
...
| MEDIUM | Data | Remove unused `mysql-connector-java` dependency (issue #29) — startup scan + driver registration | MEDIUM |
| LOW | Code | `System.out/err.println` used for audit/logging throughout | LOW |

## Backlog Cursor
- Next scan: Task 4 (PR maintenance for existing efficiency PRs), Task 6 (measurement infra)
...
- Next run should focus: Task 4 (PR maintenance), Task 6 (measurement infra)
