...
- **Pre-existing test failures** (on main): `DateUtilsTest.testGetQuarter` (date boundary), `TaskServiceTest.testGetTaskStatistics` (divide-by-zero, addressed in PR #123)
...
- GET /tasks now paginated (default size=20) — PR created 2026-08-14
- getAllTasks() (unbounded) still used internally by getTaskStatistics, exportTasksAsCsv, getOverdueTasks

## Optimisation Backlog

| Priority | Focus Area | Opportunity | Estimated Impact |
|---|---|---|---|
| HIGH | Data | `getAllTasks()` internal callers still unbounded — `getTaskStatistics`, `exportTasksAsCsv`, `getOverdueTasks` | HIGH at scale |
...
## Backlog Cursor
- Next scan: Task 5 (comments on efficiency issues), Task 6 (JMH if #96 signed off)
...
- **2026-08-13**: PR #123 created (branch: efficiency/single-pass-task-statistics) — consolidate 13 iteration passes in getTaskStatistics() into one O(n) loop; inline overdue count to eliminate 2nd findAll(); fix divide-by-zero
- **2026-08-14**: PR created (branch: efficiency/paginate-get-all-tasks) — add pagination to GET /tasks endpoint, default page size 20; ~50× payload reduction at 1000 tasks

## Tasks Last Run (for round-robin)
- 2026-08-13: Task 3, Task 5, Task 7
- 2026-08-14: Task 3, Task 4, Task 7
- Next run should focus: Task 5 (efficiency issue comments), Task 6 (JMH infrastructure)
